...
function loadDataTable() {
    dataTable = $('#DT_load').DataTable({
        ...
        "columns": [
            { "data": "name", "width": "12%", "title": "Lieferanten" },
            {
                "data": "id",
                "render": function (data) {
                    return `<div class="text-center">
                        <a href="/FoodSuppliers/Upsert?id=${data}" class='btn btn-success text-white' style='cursor:pointer; width:80px;'>
                            Ändern
                        </a>
                        &nbsp;
                        <a class='btn btn-danger text-white' style='cursor:pointer; width:80px;'
                            onclick=Delete('/foodSuppliers/Delete?id='+${data})>
                            Löschen
                        </a>
                        </div>`;
                }, "width":"16%"
